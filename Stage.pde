...
class stage_1 extends Stage{
  ...
  void updateMe(){
    ...
    if(count == 2400){
      enemys.add(new zako_trail01(width / 2 - Play_Haba / 4, 0));
    }
    if(count == 2460){
      enemys.add(new zako_trail01(width / 2 , 0));
    }
    if(count == 2520){
      enemys.add(new zako_trail01(width / 2 + Play_Haba / 4, 0));
    }
    if(count == 2720 && isCounting == true){
      isCounting = false;
      enemys.add(new midboss_geometric(width / 2, 0));
    }
    if(count > 2720 && enemys.size() == 0 && items.size() == 0){
      endStage();
    }
  }
